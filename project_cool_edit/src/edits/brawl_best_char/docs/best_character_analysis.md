...
- **Voice Lines Architecture**:
  - **Fake Winner Contender Voice**: Played during the candidate presentation (e.g. Leon `"Sneaky time!"` voice line at F222).
  - **True Winner Reveal Multi-Voice**: Played during the climax sequence with clean timing gaps (Voice 1 `super.ogg` at F315 reveal start, Voice 2 `attack.ogg` at F345 climax text pop).

---

## ⏱ Complete Frame-by-Frame & Timestamp Breakdown

| Frame Range | Timestamp Range | Phase / Segment | Visual / FX Details | Audio / Voice Event | Text / Typography Overlay | Image Asset / Pose | Remotion Props Mapping |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `F000 → F012` | `0.000s → 0.400s` | Intro Hook Title | Rotating anime sunburst beams, blurred brawler collage backdrop, purple/pink radial aura | Background Phonk Track | `"THE BEST BRAWLER"` (White, purple glow, stroke 3.5px) | Brawler Collage Grid | `intro.headerText` |
...
| `F159 → F189` | `5.300s → 6.300s` | Contender 4: R-T | White/RGB flash burst (F159-F161), sky blue particle overlay, floating hover motion | **Cut #4 / Beat Hit (F159)** | `"R-T?"` (White, sky blue `#38bdf8` glow, stroke 3.5px) | `rt_panel_1.png` | `contenders[3]` |
...
| `F222 → F263` | `7.400s → 8.767s` | Contender 6: Leon (Image 1) | White/RGB flash burst (F222-F224), green particle overlay, floating hover motion | **Cut #6 / Leon Voice Line 1 (F222)** (`leon_ulti_vo_01.ogg`) | `"LEON?"` (White, green `#10b981` glow, stroke 3.5px) | `leon_panel_1.png` | `contenders[5].voiceLine` |
...
| `F313 → F322` | `10.433s → 10.733s` | Winner Climax: Image 1 | Climax Shockwave Shake `[intensity: 1.0]`, gold radial aura, gold particles | **Climax Beat Hit / Kenji Voice 1 (F315)** (`super.ogg`) | None | `kenji_panel_1.png` | `winner.voiceLines[0]` |
...
| `F340 → F349` | `11.333s → 11.633s` | Winner Climax: Image 4 | Flash burst on cut, gold particle aura, continuous camera shake | **Kenji Voice 2 (F345)** (`attack.ogg` after gap) | `"OFC IT'S KENJI 👑"` | `kenji_panel_8.png` | `winner.voiceLines[1]` |
...
export const customBestCharProps: BestCharEditProps = {
  ...
  contenders: [
    ...
    { id: "rt", name: "R-T", questionText: "R-T?", image: staticFile("images/rt/rt_panel_1.png"), accentColor: "#38bdf8", startFrame: 159, endFrame: 189 },
    ...
    { 
      ...
      voiceLine: staticFile("brawler_voices/leon/leon_ulti_vo_01.ogg"), // Fake-out contender voice line
      accentColor: "#10b981", 
      startFrame: 222, 
      endFrame: 313 
    },
