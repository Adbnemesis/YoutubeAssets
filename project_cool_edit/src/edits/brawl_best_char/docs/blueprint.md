...
> [!IMPORTANT]
> **RULE 1: 2nd Last Dummy Winner Mechanics (`contender.secondaryImage` & `contender.voiceLine`)**:
> - **The 2nd Last Brawler is the Dummy/Fake Winner** (e.g., Crow before Surge, or Leon before Kenji).
> - **ONLY the 2nd Last Brawler gets 2 images with a pose cut at F263** (`secondaryImage`) AND a **voice line** (`voiceLine`), serving as the fake-out winner build-up hook right before the true climax twist!
> - **Intermediate brawlers MUST NOT have voice lines or secondary pose cuts.**

> [!IMPORTANT]
> **RULE 2: True Climax Winner Mechanics (`winner.winnerPanels` & `winner.voiceLines`)**:
...
> - Gets multi-voice line triggers at reveal start (F315) and text pop (F345).
...
1. **2nd Last Dummy Winner Voice (`contender.voiceLine`)**:
   - Triggered when the 2nd last dummy winner enters at frame F222 (e.g. Crow voice line `crow/attack.ogg`).
   - Serves as the fake-out auditory hook right before the true winner climax reveal at F313.

2. **True Winner Reveal Multi-Voice (`winner.voiceLines`)**:
   - **Voice Line 1** (`surge_atk_vo_04.ogg`): Triggered at frame F315 (local frame 2) when the true winner reveal begins.
   - **Voice Line 2** (`surge_hurt_vo_05.ogg`): Triggered at frame F345 (local frame 32) after the gold announcement text pop.
...
| `F222 → F263` | `7.400s → 8.767s` | Contender 5: Crow (Dummy Winner Image 1) | RGB flash (F222-F224), blue particles, moving speed lines | **Crow Voice Line** (`crow/attack.ogg`) | `crow_panel_1.png` |
| `F263 → F313` | `8.767s → 10.433s` | Contender 5: Crow (Dummy Winner Image 2) | Secondary pose cut at F263, chromatic glitch shift (F263-F267) | Beat Hit | `crow_panel_4.png` |
...
export const newEditProps: BestCharEditProps = {
  ...
  contenders: [
    ...
    // STRICT RULE: ONLY 2ND LAST DUMMY WINNER GETS SECONDARY IMAGE & VOICE LINE!
