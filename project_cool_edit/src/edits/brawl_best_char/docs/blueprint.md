...
The `brawl_best_char` edit is a standardized vertical (9:16) Short template that ranks brawlers against a phonk audio track. It follows a strict 3-phase progression:
...
2. **Contenders Phase** (`F064 -> F313`): Sequential brawler cards showing character artwork + question text (e.g., `"LEON?"`).
   - The **2nd Last Brawler is ALWAYS the Dummy/Fake Winner** (`F222 -> F313`), featuring a voice line on entrance at `F222` and a secondary pose cut at `F263` (Beat 7).
...
> [!IMPORTANT]
> **RULE 1: Frame Continuity (NO GAPS / NO BLACK SCREENS)**
> - Every contender's `startFrame` **MUST EXACTLY EQUAL** the preceding contender's `endFrame`.
> - There must be **ZERO GAPS** between contiguous contender cards across the entire `F064 -> F313` timeline.
> - **Standard Beat Frame Anchors**: `F064`, `F096`, `F127`, `F159`, `F189`, `F222`, `F263`, `F313`, `F377`.

> [!IMPORTANT]
> **RULE 2: 2nd Last Brawler is ALWAYS the Dummy Winner (`F222 -> F313`)**
> - The 2nd to last brawler in `contenders` (e.g. Crow or Leon) **MUST start at F222 and end at F313**.
...
| `F064 → F096` | `2.133s → 3.200s` | Contender 1 | Beat #2 | Entrance flash (F64-F66), floating artwork, question text pop | Phonk Beat #2 |
| `F096 → F127` | `3.200s → 4.233s` | Contender 2 | Beat #3 | Entrance flash (F96-F98), floating artwork, question text pop | Phonk Beat #3 |
| `F127 → F159` | `4.233s → 5.300s` | Contender 3 | Beat #4 | Entrance flash (F127-F129), floating artwork, question text pop | Phonk Beat #4 |
| `F159 → F222` | `5.300s → 7.400s` | Contender 4 | Beat #5 | Entrance flash (F159-F161), floating artwork, question text pop | Phonk Beat #5 |
| `F222 → F263` | `7.400s → 8.767s` | **2nd Last Dummy Winner (Pose 1)** | **Beat #6 (Hook)** | Entrance flash burst (F222-F224), moving speed lines | **Dummy Winner Voice Line** at `F222` |
| `F263 → F313` | `8.767s → 10.433s` | **2nd Last Dummy Winner (Pose 2)** | **Beat #7 (Pose Cut)** | Secondary pose cut at `F263`, chromatic glitch burst (F263-F267) | Phonk Beat #7 |
| `F313 → F377` | `10.433s → 12.566s` | **True Climax Winner** | **Beat #8 (Drop)** | 7-panel climax reveal, climax shockwave `[intensity: 1.0]`, gold crown aura | **Winner Voice Line 1** at `F315`<br>**Winner Voice Line 2** at `F345` |
...
export const myCustomEditProps: BestCharEditProps = {
  ...
  contenders: [
    ...
    { id: "brawler4", name: "Brawler 4", questionText: "BRAWLER 4?", image: staticFile("images/brawler4/brawler4_panel_1.png"), accentColor: "#06b6d4", startFrame: 159, endFrame: 222 },
