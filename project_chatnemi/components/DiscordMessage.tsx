...
export const DiscordMessage: React.FC<{
  ...
  const isFileAttachment =
    text.includes(".zip") || text.includes(".rar") || text.includes("[FILE:");
  let fileName = "";
  let fileSize = "";
  let cleanText = text;
  ...
  if (isFileAttachment) {
    ...
    if (
      lines.length >= 2 &&
      (lines[0].includes(".zip") || lines[1].includes("GB") || lines[1].includes("MB"))
    ) {
  ...
  return (
    <div
      style={{
        display: "flex",
        flexDirection: "row",
        padding: isFirstMessageInGroup
          ? "16px 16px 2px 72px"
          : "2px 16px 2px 72px",
        position: "relative",
        marginTop: isFirstMessageInGroup ? 14 : 2,
        minHeight: isFirstMessageInGroup ? 48 : 24,
      }}
    >
      ...
      {isFirstMessageInGroup && (
        ...
        >
          {character.avatarUrl ? (
            <img
              src={staticFile(
                `project_chatnemi_assets/profile_pic/${character.avatarUrl}`
              )}
              style={{
                width: "100%",
                height: "100%",
      ...
      <div style={{ display: "flex", flexDirection: "column", width: "100%" }}>
        {/* Name & Timestamp Header (Only on first message of group) */}
        {isFirstMessageInGroup && (
