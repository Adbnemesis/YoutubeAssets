...
// ─────────────────────────────────────────────────────────────
// NEMI EXPLAINS V4 — PURE CHATTERBOX SPEECH SYNCHRONIZATION
// ─────────────────────────────────────────────────────────────
...
const TRACE_CONNECTIONS = [
  { fromId: 0, toId: 1, startFrame: 340 }, // ROOT -> app
  { fromId: 1, toId: 2, startFrame: 365 }, // app -> user
  { fromId: 1, toId: 6, startFrame: 390 }, // app -> db_client
  { fromId: 2, toId: 4, startFrame: 415 }, // user -> jwt_token
  { fromId: 6, toId: 8, startFrame: 440 }, // db_client -> socket
  { fromId: 6, toId: 10, startFrame: 465 }, // db_client -> session_cache
];

// The Surprise Connection (from user -> hidden_ref)
const SURPRISE_CONNECTION = { fromId: 2, toId: 12, startFrame: 560 };
...
export const NemiExplainsV4Comp: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // ─── Nemi Dynamic Pose Progression ───
  let nemiPose: NemiPose = "thinking";
  if (frame < 45) nemiPose = "thinking";
  else if (frame < 140) nemiPose = "shocked";
  else if (frame < 260) nemiPose = "puzzled";
  else if (frame < 330) nemiPose = "thinking";
  else if (frame < 520) nemiPose = "thinking";
  else if (frame < 560) nemiPose = "puzzled"; // looking at isolated node
  else if (frame < 610) nemiPose = "aha"; // surprise connection found!
  else if (frame < 680) nemiPose = "aha"; // watching garbage purge
  else nemiPose = "smug"; // proud, clean memory!

  // ─── Nemi Spoken Dialogue Bubbles (Synchronized to Nemi Voice Tracks) ───
  let nemiSpeech = "";
  if (frame >= 70 && frame < 150) nemiSpeech = "Uh... that's a lot.";
  else if (frame >= 170 && frame < 260) nemiSpeech = "Who gets deleted?";
  else if (frame >= 510 && frame < 555) nemiSpeech = "That one looks dead.";
  else if (frame >= 560 && frame < 610) nemiSpeech = "Oh! It's connected!";
  else if (frame >= 635 && frame < 685) nemiSpeech = "Bye! 👋";
  else if (frame >= 810 && frame < 940) nemiSpeech = "Much better. 😎";
  ...
  if (frame >= 345) reachedSet.add(1); // app
  if (frame >= 375) reachedSet.add(2); // user
  if (frame >= 400) reachedSet.add(6); // db_client
  if (frame >= 425) reachedSet.add(4); // jwt_token
  if (frame >= 450) reachedSet.add(8); // socket
  if (frame >= 475) reachedSet.add(10); // session_cache
  if (frame >= 575) reachedSet.add(12); // hidden_ref (The Surprise!)

  // ─── Compaction Spring ───
  const compactSpring = frame >= 680
    ? spring({ frame: frame - 680, fps, config: { damping: 14, stiffness: 120, mass: 0.9 } })
    : 0;
    ...
    >
      ...
      >
        ...
        {frame >= 330 && (
          <div
            style={{
              ...
              opacity: interpolate(frame, [330, 340], [0, 1], { extrapolateRight: "clamp" }),
            }}
          >
            <span
              style={{
        ...
        {OBJECTS.map((obj) => {
          ...
          const isTracing = frame >= 330;
          const isSweeping = frame >= 610;
          ...
          if (isGarbage && isSweeping) {
            garbageScale = interpolate(frame, [610, 675], [1, 0], {
              extrapolateLeft: "clamp",
              extrapolateRight: "clamp",
            });
            garbageOpacity = interpolate(frame, [610, 665], [1, 0], {
              extrapolateLeft: "clamp",
              extrapolateRight: "clamp",
            });
          }
          if (isGarbage && frame >= 675) return null;

          // Compaction Position
          let finalCx = cx;
          let finalCy = cy;
          if (obj.alive && frame >= 680) {
              ...
              {isTracing && (
                <span
                  style={{
                    fontSize: 11,
                    fontWeight: 800,
                    color: isReached ? "#34D399" : (frame >= 480 ? "#FB7185" : "#94A3B8"),
                    letterSpacing: 0.8,
                    fontFamily: NEMI_THEME.typography.fontHeading,
                  }}
                >
                  {isReached ? "✓ REACHABLE" : (frame >= 480 ? "✕ DEAD" : "CHECKING")}
                </span>
              )}
            </div>
          );
        ...
        {frame >= 770 && (
          <div
            style={{
              ...
              display: "flex",
              justifyContent: "center",
              opacity: interpolate(frame, [770, 780], [0, 1], { extrapolateRight: "clamp" }),
            }}
          >
            <div
              style={{
      ...
      <Audio src={staticFile("sounds/nemi_v4_master_audio.mp3")} volume={1.0} />
