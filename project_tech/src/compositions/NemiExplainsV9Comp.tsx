...
import cuesData from "../data/nemi_v9_cues.json";

// ═══════════════════════════════════════════════════════════════════
// NEMI EXPLAINS V09 — MASTER CONTINUOUS CAMERA CHOREOGRAPHY
// STRICT SPEAKER ORCHESTRATION & ZERO-OVERLAP ENGINE (~21.5s @ 30fps)
// ═══════════════════════════════════════════════════════════════════
...
const TREE_NODES = [
  { id: 1, label: "app", type: "OBJECT", x: 320, y: 660, w: 210, h: 80, parentX: 540, parentY: 440 + 76, delayOffset: 4 },
  { id: 2, label: "user", type: "OBJECT", x: 760, y: 660, w: 210, h: 80, parentX: 540, parentY: 440 + 76, delayOffset: 8 },
  { id: 3, label: "db_client", type: "CLIENT", x: 190, y: 900, w: 170, h: 78, parentX: 320, parentY: 660 + 80, delayOffset: 14 },
  { id: 4, label: "jwt_token", type: "STRING", x: 420, y: 900, w: 170, h: 78, parentX: 320, parentY: 660 + 80, delayOffset: 18 },
  { id: 5, label: "socket", type: "NET", x: 660, y: 900, w: 170, h: 78, parentX: 760, parentY: 660 + 80, delayOffset: 22 },
  { id: 6, label: "hidden_ref", type: "CLOSURE", x: 890, y: 900, w: 170, h: 78, parentX: 760, parentY: 660 + 80, delayOffset: 26 },
];
...
export const NemiExplainsV9Comp: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // ─── Timeline Events Derived from V9 Speaker Orchestration ───
  const evHook = getEvent("v9_narrator_01_hook");
  const evChallenge = getEvent("v9_narrator_02_challenge");
  const evGuess = getEvent("v9_nemi_01_guess");
  const evWait = getEvent("v9_narrator_02b_wait");
  const evDiscovery = getEvent("v9_narrator_03_discovery");
  const evRule = getEvent("v9_narrator_04_rule");
  const evBye = getEvent("v9_nemi_02_bye");
  const evPayoff = getEvent("v9_narrator_05_payoff");
  const evBetter = getEvent("v9_nemi_03_better");

  // Semantic Phrase Timing
  const fRamSurge = getCueFrame("v9_narrator_01_hook", "ram_surge", evHook.start_frame + 45);
  const fQuestion = getCueFrame("v9_narrator_01_hook", "question_mark", evHook.start_frame + 80);
  const fGrid = getCueFrame("v9_narrator_02_challenge", "challenge_grid", evChallenge.start_frame + 15);
  const fViewerPrompt = getCueFrame("v9_narrator_02_challenge", "viewer_prompt", evChallenge.start_frame + 45);
  const fPoint = evGuess.start_frame;
  const fFreeze = evWait.start_frame;
  const fLaser = getCueFrame("v9_narrator_02b_wait", "laser_reveal", evWait.start_frame + 18);
  const fZoomRoot = getCueFrame("v9_narrator_03_discovery", "camera_zoom_root", evDiscovery.start_frame + 10);
  const fFollowEdge = getCueFrame("v9_narrator_03_discovery", "camera_follow_edge", evDiscovery.start_frame + 28);
  const fPullBack = getCueFrame("v9_narrator_04_rule", "camera_pull_back", evRule.start_frame + 8);
  const fVaporize = evBye.start_frame;
  const fCompactSnap = getCueFrame("v9_narrator_05_payoff", "compaction_snap", evPayoff.start_frame + 16);
  const fPayoffSummary = getCueFrame("v9_narrator_05_payoff", "final_summary", evPayoff.start_frame + 42);
  ...
  if (isDiscoveryScene) {
    ...
    cameraZoom = interpolate(frame, [evDiscovery.start_frame, fZoomRoot, fFollowEdge], [1.0, 1.25, 1.15], {
    ...
    cameraZoom = interpolate(frame, [evRule.start_frame, fPullBack + 10], [1.15, 0.95], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
    });
    cameraPanY = interpolate(frame, [evRule.start_frame, fPullBack + 10], [-120, 0], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
    });
  }

  // ─── Nemi Dynamic State & Non-Overlapping Dialogue ───
  ...
  if (isHookScene) {
    ...
    nemiY = 1620;
    nemiScale = 1.55;
    if (isPointing && frame < evGuess.end_frame + 10) {
      nemiSpeech = "That one! 👉";
      speechStartFrame = fPoint;
    }
  } else if (isFreezeScene) {
    const hasLaser = frame >= fLaser;
    nemiPose = hasLaser ? "shocked" : "puzzled";
    nemiX = 220;
    nemiY = 1620;
    nemiScale = 1.55;
    ...
    nemiY = 1640;
    nemiScale = 1.55;
    if (frame >= evBye.start_frame && frame < evBye.end_frame + 12) {
      nemiSpeech = "Bye. 👋🧹";
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: bgColor, overflow: "hidden", fontFamily: NEMI_THEME.typography.fontDisplay }}>
      ...
      {/* ═══════════════════════════════════════════════════════════
          SCENE 1 — HOOK & PROBLEM (Continuous Narration Block 1)
         ═══════════════════════════════════════════════════════════ */}
      {isHookScene && (() => {
        const isSurging = frame >= fRamSurge;
        const isAsking = frame >= fQuestion;
        const ramMB = Math.min(512, Math.floor(interpolate(frame, [0, evChallenge.start_frame], [14, 492], { extrapolateRight: "clamp" })));
        ...
        return (
          <>
            ...
            {/* Cascading Memory Blocks Spawning Organically */}
            {!isAsking &&
      ...
      {/* ═══════════════════════════════════════════════════════════
          SCENE 2 — VIEWER CHALLENGE & NEMI GUESS (750ms Mental Gap)
         ═══════════════════════════════════════════════════════════ */}
      {isChallengeScene && (() => {
        const isPointing = frame >= fPoint;
      ...
      {/* ═══════════════════════════════════════════════════════════
         ...
         ═══════════════════════════════════════════════════════════ */}
      {isFreezeScene && (() => {
        const lineProgress = frame >= fLaser ? interpolate(frame, [fLaser, fLaser + 14], [0, 1], { extrapolateRight: "clamp" }) : 0;
      ...
      {/* ═══════════════════════════════════════════════════════════
          SCENES 4 & 5 — CONTINUOUS CAMERA JOURNEY (Roots & Rule)
         ═══════════════════════════════════════════════════════════ */}
        ...
        return (
          <>
            <div style={{ position: "absolute", top: 160, left: 60, right: 60, textAlign: "center", zIndex: 30 }}>
              ...
              >
                {isRuleActive ? "Anything it can't... is garbage. 💥" : "V8 starts from roots & follows connections..."}
              </h2>
            </div>
            ...
            >
              ...
              <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 10 }}>
                {TREE_NODES.map((node) => {
                  ...
                  const progress = nodeActive ? Math.min(1, (frame - nodeTrigger) / 8) : 0;
      ...
      {/* ═══════════════════════════════════════════════════════════
          SCENE 7 — COMPACTION: DEFRAGMENTATION
         ═══════════════════════════════════════════════════════════ */}
      {isCompactScene && (() => {
      ...
      {/* ═══════════════════════════════════════════════════════════
          SCENE 8 — PAYOFF & CLEAN SUMMARY (Narration Block 5)
         ═══════════════════════════════════════════════════════════ */}
      {isPayoffScene && (() => {
      ...
      {/* ═══════════════════════════════════════════════════════════
         ...
         ═══════════════════════════════════════════════════════════ */}
      <Audio src={staticFile("sounds/nemi_v9_master_audio.mp3")} volume={1.0} />
