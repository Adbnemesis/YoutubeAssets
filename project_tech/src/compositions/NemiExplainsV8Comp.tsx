...
import cuesData from "../data/nemi_v8_cues.json";

// ═══════════════════════════════════════════════════════════════════
// NEMI EXPLAINS V08 — MASTER MICRO-STORY & COHERENT PERFORMANCE
// CONTINUOUS SEMANTIC CHOREOGRAPHY (Target ~22.5s @ 30fps)
// ═══════════════════════════════════════════════════════════════════

const getBlock = (id: string) => {
  const b = cuesData.narration_blocks.find((x) => x.id === id);
  return b ?? { start_frame: 0, end_frame: 0, start_time_ms: 0, end_time_ms: 0, duration_s: 0, semantic_cues: [] };
};

const getCueFrame = (blockId: string, cueName: string, fallback: number) => {
  const b = cuesData.narration_blocks.find((x) => x.id === blockId);
  if (!b) return fallback;
  const sc = b.semantic_cues.find((x: any) => x.cue === cueName);
  return sc ? sc.frame : fallback;
};

// Cascading Heap Allocation Objects for Beat 1
const HOOK_OBJECTS = [
...
const CHALLENGE_CARDS = [
  { id: "user", label: "user_session", type: "ACTIVE ROOT", addr: "0x7FFE81", size: "48 KB", x: 300, y: 600 },
  { id: "orphan", label: "orphan_cache", type: "DANGLING REF", addr: "0x7FFE94", size: "120 KB", x: 780, y: 600 },
  { id: "db", label: "db_client", type: "SOCKET CONN", addr: "0x7FFEAA", size: "250 KB", x: 300, y: 880 },
  { id: "hidden", label: "hidden_ref", type: "CLOSURE TRAP", addr: "0x7FFEC2", size: "64 KB", x: 780, y: 880, isSurprise: true },
];

// Tree Node Coordinates (X: Center, Y: Top)
const ROOT_NODE = { x: 540, y: 440, w: 260, h: 76, label: "GLOBAL ROOT" };
const TREE_NODES = [
  { id: 1, label: "app", type: "OBJECT", x: 320, y: 660, w: 210, h: 80, parentX: 540, parentY: 440 + 76, delayOffset: 5 },
  { id: 2, label: "user", type: "OBJECT", x: 760, y: 660, w: 210, h: 80, parentX: 540, parentY: 440 + 76, delayOffset: 12 },
  { id: 3, label: "db_client", type: "CLIENT", x: 190, y: 900, w: 170, h: 78, parentX: 320, parentY: 660 + 80, delayOffset: 20 },
  { id: 4, label: "jwt_token", type: "STRING", x: 420, y: 900, w: 170, h: 78, parentX: 320, parentY: 660 + 80, delayOffset: 26 },
  { id: 5, label: "socket", type: "NET", x: 660, y: 900, w: 170, h: 78, parentX: 760, parentY: 660 + 80, delayOffset: 32 },
  { id: 6, label: "hidden_ref", type: "CLOSURE", x: 890, y: 900, w: 170, h: 78, parentX: 760, parentY: 660 + 80, delayOffset: 38 },
];
...
export const NemiExplainsV8Comp: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // ─── Semantic Trigger Frames Derived from V8 Performance Data ───
  const bHook = getBlock("v8_narrator_01_hook");
  const bNope = getBlock("v8_nemi_01_nope");
  const bChallenge = getBlock("v8_narrator_02_challenge");
  const bPoint = getBlock("v8_nemi_02_point");
  const bWait = getBlock("v8_narrator_02b_wait");
  const bOh = getBlock("v8_nemi_03_oh");
  const bDiscovery = getBlock("v8_narrator_03_discovery");
  const bCleanup = getBlock("v8_narrator_04_cleanup");
  const bBye = getBlock("v8_nemi_04_bye");
  const bPayoff = getBlock("v8_narrator_05_payoff");
  const bBetter = getBlock("v8_nemi_05_better");

  // Semantic Phrase Timing
  const fRamSurge = getCueFrame("v8_narrator_01_hook", "ram_surge", bHook.start_frame + 45);
  const fQuestion = getCueFrame("v8_narrator_01_hook", "question_mark", bHook.start_frame + 80);
  const fGrid = getCueFrame("v8_narrator_02_challenge", "challenge_grid", bChallenge.start_frame + 20);
  const fPoint = bPoint.start_frame;
  const fFreeze = bWait.start_frame;
  const fLaser = getCueFrame("v8_narrator_02b_wait", "laser_reveal", bWait.start_frame + 18);
  const fTree = bDiscovery.start_frame;
  const fRoots = getCueFrame("v8_narrator_03_discovery", "root_pulse", bDiscovery.start_frame + 15);
  const fTraverse = getCueFrame("v8_narrator_03_discovery", "tree_traversal", bDiscovery.start_frame + 40);
  const fRule = bCleanup.start_frame;
  const fClimax = bBye.start_frame;
  const fCompact = getCueFrame("v8_narrator_05_payoff", "compaction_slide", bPayoff.start_frame + 20);
  const fPayoffSummary = getCueFrame("v8_narrator_05_payoff", "final_summary", bPayoff.start_frame + 45);

  // ─── Scene State Classification ───
  const isHookScene = frame < bChallenge.start_frame;
  const isChallengeScene = frame >= bChallenge.start_frame && frame < bWait.start_frame;
  const isFreezeScene = frame >= bWait.start_frame && frame < bDiscovery.start_frame;
  const isDiscoveryScene = frame >= bDiscovery.start_frame && frame < bCleanup.start_frame;
  const isRuleScene = frame >= bCleanup.start_frame && frame < bBye.start_frame;
  const isClimaxScene = frame >= bBye.start_frame && frame < fCompact;
  const isCompactScene = frame >= fCompact && frame < fPayoffSummary;
  ...
  if (isHookScene) {
    ...
    nemiX = isAsking ? 220 : 880;
    nemiY = 1620;
    nemiScale = 1.5;
    if (frame >= bNope.start_frame && frame < bNope.end_frame + 12) {
      nemiSpeech = "Because I'm not doing it. 🙅‍♂️";
      speechStartFrame = bNope.start_frame;
    }
    ...
    nemiY = 1620;
    nemiScale = 1.55;
    if (isPointing && frame < bPoint.end_frame + 12) {
      nemiSpeech = "That one! 👉";
      speechStartFrame = fPoint;
    }
  } else if (isFreezeScene) {
    const hasRealized = frame >= bOh.start_frame;
    nemiPose = hasRealized ? "shocked" : "puzzled";
    nemiX = 220;
    nemiY = 1620;
    nemiScale = 1.55;
    if (hasRealized && frame < bOh.end_frame + 15) {
      nemiSpeech = "Wait... Oh. 🤯";
      speechStartFrame = bOh.start_frame;
    }
    ...
    nemiY = 1640;
    nemiScale = 1.55;
    if (frame >= bBye.start_frame && frame < bBye.end_frame + 15) {
      nemiSpeech = "Bye. 👋🧹";
      speechStartFrame = bBye.start_frame;
    }
    ...
    nemiY = 1580;
    nemiScale = 1.65;
    if (frame >= bBetter.start_frame) {
      nemiSpeech = "Much better! 😎✨";
      speechStartFrame = bBetter.start_frame;
    }
  }
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: bgColor, overflow: "hidden", fontFamily: NEMI_THEME.typography.fontDisplay }}>
      ...
      {/* Dynamic Background Texture & Lighting */}
      {isDarkScene ? (
        <>
          <div
            style={{
      ...
      {/* ═══════════════════════════════════════════════════════════
          SCENE 1 — HOOK & THE QUESTION (Continuous Narration Block 1)
         ═══════════════════════════════════════════════════════════ */}
      {isHookScene && (() => {
        const isSurging = frame >= fRamSurge;
        const isAsking = frame >= fQuestion;
        const ramMB = Math.min(512, Math.floor(interpolate(frame, [0, bNope.start_frame], [14, 492], { extrapolateRight: "clamp" })));
        ...
        return (
          <>
            ...
            >
              ...
              >
                {isAsking ? "So who cleans all of this up?" : "Your JavaScript keeps creating objects..."}
              </h1>
              ...
              {isSurging && !isAsking && (
                ...
                >
                  <span style={{ fontSize: 60, fontWeight: 900, color: "#E11D48", letterSpacing: -2 }}>
                    A lot of them. 💥
                  </span>
                </div>
              )}
            ...
            {/* Cascading Objects Spawning Organically */}
            {!isAsking &&
              HOOK_OBJECTS.map((item, i) => {
                const local = frame - item.delay;
                if (local < 0) return null;
                const pop = spring({ frame: local, fps, config: { damping: 9, stiffness: 250, mass: 0.5 } });
            ...
            {/* Context Card during the Question Phrase */}
            {isAsking && (
              <div
                style={{
                  position: "absolute",
      ...
      {/* ═══════════════════════════════════════════════════════════
          SCENE 2 — VIEWER CHALLENGE & NEMI GUESS (Narration Block 2)
         ═══════════════════════════════════════════════════════════ */}
      {isChallengeScene && (() => {
        const isPointing = frame >= fPoint;
        const pop = spring({ frame: frame - bChallenge.start_frame, fps, config: NEMI_THEME.springs.pop });
      ...
      {/* ═══════════════════════════════════════════════════════════
          SCENE 3 — FREEZE & DISCOVERY REVEAL (Laser Connector)
         ═══════════════════════════════════════════════════════════ */}
      {isFreezeScene && (() => {
        const local = frame - bWait.start_frame;
        const lineProgress = frame >= fLaser ? interpolate(frame, [fLaser, fLaser + 15], [0, 1], { extrapolateRight: "clamp" }) : 0;
      ...
      {/* ═══════════════════════════════════════════════════════════
          SCENES 4 & 5 — ROOT TRAVERSAL & THE RULE (Blocks 3 & 4)
         ═══════════════════════════════════════════════════════════ */}
        ...
        return (
          <>
            <div style={{ position: "absolute", top: 160, left: 60, right: 60, textAlign: "center", zIndex: 30 }}>
              ...
              >
                {isRuleActive ? "Unreachable is garbage. It gets removed! 💥" : "V8 starts from roots & follows connections..."}
              </h2>
            </div>
            ...
            <div style={{ position: "absolute", top: 320, left: 40, right: 40, height: 950, zIndex: 20 }}>
              ...
              <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 10 }}>
                {TREE_NODES.map((node) => {
                  const nodeTrigger = fTree + node.delayOffset;
                  const nodeActive = frame >= nodeTrigger;
                  const progress = nodeActive ? Math.min(1, (frame - nodeTrigger) / 10) : 0;
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14 }}>
                  <span style={{ fontSize: 12, fontWeight: 900, color: isRuleActive ? "#FB7185" : "#64748B", letterSpacing: 1.5 }}>
                    ISOLATED HEAP OBJECTS (0 ROOT LINKS)
                  </span>
                  <span style={{ fontSize: 12, fontWeight: 900, color: isRuleActive ? "#FB7185" : "#64748B" }}>
      ...
      {/* ═══════════════════════════════════════════════════════════
          SCENE 6 — RAPID CLEANUP CLIMAX (Nemi "Bye.")
         ═══════════════════════════════════════════════════════════ */}
      {isClimaxScene && (() => {
        const local = frame - bBye.start_frame;
      ...
      {/* ═══════════════════════════════════════════════════════════
          SCENE 7 — COMPACTION: PHYSICAL DEFRAGMENTATION
         ═══════════════════════════════════════════════════════════ */}
      {isCompactScene && (() => {
        const local = frame - fCompact;
      ...
      {/* ═══════════════════════════════════════════════════════════
          SCENE 8 — PAYOFF & GRAND SUMMARY (Narration Block 5)
         ═══════════════════════════════════════════════════════════ */}
      {isPayoffScene && (() => {
      ...
      {/* ═══════════════════════════════════════════════════════════
         ...
         ═══════════════════════════════════════════════════════════ */}
      <Audio src={staticFile("sounds/nemi_v8_master_audio.mp3")} volume={1.0} />
      <Audio src={staticFile("sounds/sub_impact.wav")} volume={0.25} />
      {frame >= bBye.start_frame && frame < bBye.start_frame + 25 && (
