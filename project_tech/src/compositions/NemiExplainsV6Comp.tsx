...
import cues from "../data/nemi_v6_cues.json";

// ═══════════════════════════════════════════════════════════════════
// NEMI EXPLAINS V6 — MASTERPIECE SCENE-DRIVEN TECH STORYTELLING
// ATTENTION → CURIOSITY → PARTICIPATION → SURPRISE → PAYOFF
// ═══════════════════════════════════════════════════════════════════
...
// ─── Beat Timing Boundaries ───
const BEATS = {
  hook:        { start: 0,   end: 95 },
  curiosity:   { start: 96,  end: 174 },
  challenge:   { start: 175, end: 241 },
  freeze:      { start: 242, end: 325 },
  investigate: { start: 326, end: 408 },
  rule:        { start: 409, end: 530 },
  climax:      { start: 531, end: 551 },
  compact:     { start: 552, end: 600 },
  payoff:      { start: 601, end: 718 },
  end:         { start: 719, end: cues.total_frames },
};

// Hook items with rich code/tech metadata
const HOOK_ITEMS = [
...
const TREE_NODES = [
  { id: 1, label: "app", type: "OBJECT", x: 320, y: 660, w: 210, h: 80, parentX: 540, parentY: 440 + 76, delay: 335 },
  { id: 2, label: "user", type: "OBJECT", x: 760, y: 660, w: 210, h: 80, parentX: 540, parentY: 440 + 76, delay: 350 },
  { id: 3, label: "db_client", type: "CLIENT", x: 190, y: 900, w: 170, h: 78, parentX: 320, parentY: 660 + 80, delay: 368 },
  { id: 4, label: "jwt_token", type: "STRING", x: 420, y: 900, w: 170, h: 78, parentX: 320, parentY: 660 + 80, delay: 382 },
  { id: 5, label: "socket", type: "NET", x: 660, y: 900, w: 170, h: 78, parentX: 760, parentY: 660 + 80, delay: 395 },
  { id: 6, label: "hidden_ref", type: "CLOSURE", x: 890, y: 900, w: 170, h: 78, parentX: 760, parentY: 660 + 80, delay: 412 },
];
...
export const NemiExplainsV6Comp: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // ─── Current Beat Detection ───
  const isHook = frame < BEATS.hook.end;
  const isCuriosity = frame >= BEATS.curiosity.start && frame < BEATS.curiosity.end;
  ...
  const isInvestigate = frame >= BEATS.investigate.start && frame < BEATS.investigate.end;
  ...
  const isEnd = frame >= BEATS.end.start;

  // ─── Background Theme Selection ───
  const isDarkScene = isChallenge || isFreeze || isInvestigate || isRule || isClimax || isCompact;
  ...
  if (isHook) {
    ...
    nemiY = 1600;
    nemiScale = 1.55;
    if (frame >= seg("v6_004_nemi_nope").start_frame && frame < seg("v6_004_nemi_nope").end_frame + 12) {
      nemiSpeech = "Because I'm not doing it. 🙅‍♂️";
      speechStartFrame = seg("v6_004_nemi_nope").start_frame;
    }
  } else if (isChallenge) {
    const isPointing = frame >= seg("v6_006_nemi_point").start_frame;
    nemiPose = isPointing ? "pointing" : "thinking";
    nemiX = 880;
    nemiY = 1620;
    nemiScale = 1.55;
    if (isPointing && frame < seg("v6_006_nemi_point").end_frame + 12) {
      nemiSpeech = "That one! 👉";
      speechStartFrame = seg("v6_006_nemi_point").start_frame;
    }
  } else if (isFreeze) {
    const hasRealized = frame >= seg("v6_009_nemi_oh").start_frame;
    nemiPose = hasRealized ? "shocked" : "puzzled";
    nemiX = 220;
    nemiY = 1620;
    nemiScale = 1.55;
    if (hasRealized && frame < seg("v6_009_nemi_oh").end_frame + 15) {
      nemiSpeech = "Wait... Oh. 🤯";
      speechStartFrame = seg("v6_009_nemi_oh").start_frame;
    }
  } else if (isInvestigate || isRule) {
    nemiPose = "explaining";
    nemiX = 180;
    nemiY = 1660;
    nemiScale = 1.35;
    ...
    nemiY = 1640;
    nemiScale = 1.55;
    if (frame >= seg("v6_015_nemi_bye").start_frame && frame < seg("v6_015_nemi_bye").end_frame + 15) {
      nemiSpeech = "Bye. 👋🧹";
      speechStartFrame = seg("v6_015_nemi_bye").start_frame;
    }
  } else if (isCompact) {
    nemiPose = "smug";
    nemiX = 880;
    nemiY = 1640;
    nemiScale = 1.4;
  } else if (isPayoff || isEnd) {
    nemiPose = "aha";
    nemiX = 540;
    nemiY = 1580;
    nemiScale = 1.65;
    if (frame >= seg("v6_019_nemi_better").start_frame) {
      nemiSpeech = "Much better! 😎✨";
      speechStartFrame = seg("v6_019_nemi_better").start_frame;
    }
  }
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: bgColor, overflow: "hidden", fontFamily: NEMI_THEME.typography.fontDisplay }}>
      ...
      {/* Dynamic Background Grid & Ambient Lighting */}
      {isDarkScene ? (
        <>
          <div
            style={{
      ...
      {isHook && (() => {
        ...
        const isALot = frame >= seg("v6_002_alot").start_frame;
        const ramMB = Math.min(512, Math.floor(interpolate(frame, [0, 90], [12, 480], { extrapolateRight: "clamp" })));
            ...
            >
              ...
              {isALot && (
                <div
                  style={{
                    marginTop: 14,
                    transform: `scale(${spring({ frame: frame - seg("v6_002_alot").start_frame, fps, config: NEMI_THEME.springs.pop })})`,
                  }}
                >
                  <span
                    style={{
      ...
      {/* ═══════════════════════════════════════════════════════════
          BEAT 2 — CURIOSITY: THE MILLION DOLLAR QUESTION
         ═══════════════════════════════════════════════════════════ */}
      {isCuriosity && (() => {
        const local = frame - BEATS.curiosity.start;
      ...
      {/* ═══════════════════════════════════════════════════════════
          BEAT 3 — CHALLENGE: AUDIENCE PARTICIPATION (2x2 Grid)
         ═══════════════════════════════════════════════════════════ */}
      {isChallenge && (() => {
      ...
      {/* ═══════════════════════════════════════════════════════════
          BEAT 4 — FREEZE / SURPRISE: HIDDEN CLOSURE REVEAL
         ═══════════════════════════════════════════════════════════ */}
      {isFreeze && (() => {
        const local = frame - BEATS.freeze.start;
        const revealStart = seg("v6_008_connected").start_frame - BEATS.freeze.start;
        ...
        return (
          <>
            <div style={{ position: "absolute", top: 180, left: 60, right: 60, textAlign: "center", zIndex: 30 }}>
              ...
              >
                {lineProgress > 0.5 ? "REACHABILITY DISCOVERY" : "WAIT A SECOND..."}
              </span>
              <h2
                style={{
                  fontSize: 58,
              ...
              >
                {lineProgress > 0.5 ? "IT'S STILL CONNECTED!" : "BUT WAIT."}
              </h2>
            </div>
            ...
            <div style={{ position: "absolute", top: 400, left: 80, right: 80, height: 750, zIndex: 20 }}>
              ...
              {/* Exact Laser SVG Connector from Bottom of user_session (x: 250, y: 240) to Top of hidden_ref (x: 650, y: 440) */}
      ...
      {/* ═══════════════════════════════════════════════════════════
          BEATS 5 & 6 — INVESTIGATE & RULE: FULL GRAPH TRAVERSAL
         ═══════════════════════════════════════════════════════════ */}
      {(isInvestigate || isRule) && (() => {
        const isRulePhase = isRule;
        const isCantReach = frame >= seg("v6_013_cant").start_frame;
        ...
        return (
          <>
            <div style={{ position: "absolute", top: 160, left: 60, right: 60, textAlign: "center", zIndex: 30 }}>
              ...
              >
                {isRulePhase ? "THE GOLDEN RULE OF GC" : "MARK-AND-SWEEP ALGORITHM"}
              </span>
              <h2
                style={{
                  fontSize: 52,
              ...
              >
                {!isRulePhase ? "V8 traces from Root Objects..." : isCantReach ? "If it can't reach... IT GOES! 💥" : "Anything it can reach STAYS! ✓"}
              </h2>
            </div>
        ...
        return (
          <>
            ...
            >
              ...
              >
                It finds what's alive... and clears the rest.
              </p>
            </div>
      ...
      {/* ═══════════════════════════════════════════════════════════
         ...
         ═══════════════════════════════════════════════════════════ */}
      <Audio src={staticFile("sounds/nemi_v6_master_audio.mp3")} volume={1.0} />
