...
// ═══════════════════════════════════════════════════════════════════
// NEMI EXPLAINS V12 — WHY DOES 0.1 + 0.2 NOT EQUAL 0.3?
// MYSTERY ARCHETYPE & CONTINUOUS WORLD ENGINE (~21s @ 30fps)
// ═══════════════════════════════════════════════════════════════════
...
export const NemiExplainsV12Comp: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // ─── Timeline Events Derived from V12 Speaker Orchestration ───
  ...
  const fCodeEval = getCueFrame("v12_narrator_01_hook", "code_eval", evHook.start_frame + 20);
  const fErrorPop = getCueFrame("v12_narrator_01_hook", "error_result_pop", evHook.start_frame + 45);
  const fMysteryCard = getCueFrame("v12_narrator_02_question", "mystery_question_card", evQuestion.start_frame + 15);
  ...
  const fAhaTransform = getCueFrame("v12_narrator_05_aha", "aha_transformation", evAha.start_frame + 20);
  const fMasterTakeaway = getCueFrame("v12_narrator_05_aha", "master_takeaway", evAha.start_frame + 50);

  // ─── Visual Stages Classification (Mystery Archetype) ───
  ...
  if (isApproxStage) {
    cameraZoom = interpolate(frame, [evApprox.start_frame, fTrailingFour], [1.0, 1.15], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
    });
    cameraPanY = interpolate(frame, [evApprox.start_frame, fTrailingFour], [0, -40], {
      extrapolateLeft: "clamp",
      extrapolateRight: "clamp",
    });
  }
  ...
  if (isHookStage) {
    const isConfused = frame >= evWhat.start_frame;
    nemiPose = isConfused ? "shocked" : "thinking";
    nemiX = 860;
    nemiY = 1620;
    nemiScale = 1.5;
    if (isConfused && frame < evWhat.end_frame + 10) {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: bgColor, overflow: "hidden", fontFamily: NEMI_THEME.typography.fontDisplay }}>
      ...
      {/* ═══════════════════════════════════════════════════════════
          BEAT 1 — THE IMPOSSIBLE RESULT (0.0–4.0s)
         ═══════════════════════════════════════════════════════════ */}
      {isHookStage && (() => {
      ...
      {/* ═══════════════════════════════════════════════════════════
          BEAT 2 — THE MYSTERY QUESTION (4.0–7.5s)
         ═══════════════════════════════════════════════════════════ */}
      {isQuestionStage && (() => {
          ...
          <>
            ...
            >
              ...
              <p style={{ fontSize: 20, fontWeight: 700, color: "#64748B", margin: 0 }}>
                It's not a software bug. It's a fundamental law of computer hardware.
              </p>
            </div>
          </>
        );
          ...
          <>
            <div style={{ position: "absolute", top: 160, left: 60, right: 60, textAlign: "center", zIndex: 30 }}>
              ...
              <h2 style={{ fontSize: 52, fontWeight: 900, color: "#FFFFFF", letterSpacing: -1.5, margin: 0 }}>
                0.1 is an infinite fraction in binary!
              </h2>
            </div>

            {/* Binary Fraction Card */}
            <div
              style={{
                position: "absolute",
                top: 400,
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 16, fontWeight: 900, color: "#94A3B8" }}>DECIMAL FRACTION</span>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: 16, fontWeight: 900, color: NEMI_THEME.colors.brand.cyanGlow }}>BINARY (BASE-2)</span>
                <span style={{ fontSize: 22, fontWeight: 900, color: NEMI_THEME.colors.brand.cyanGlow, fontFamily: NEMI_THEME.typography.fontCode }}>
                  0.0001100110011...
                </span>
              </div>
              ...
              {hasExpanded && (
                ...
                >
                  🔁 Just like 1/3 in decimal (0.3333...), 0.1 never ends in binary!
                </div>
              )}
            </div>
          </>
      ...
      {/* ═══════════════════════════════════════════════════════════
          BEAT 4 — 53-BIT APPROXIMATION & ADDITION (12.0–16.5s)
         ═══════════════════════════════════════════════════════════ */}
      {isApproxStage && (() => {
        ...
        return (
          <>
            <div style={{ position: "absolute", top: 160, left: 60, right: 60, textAlign: "center", zIndex: 30 }}>
              ...
              <h2 style={{ fontSize: 52, fontWeight: 900, color: "#FFFFFF", letterSpacing: -1.5, margin: 0 }}>
                Rounds to 53 bits of precision...
              </h2>
            </div>

            {/* Float Precision Addition Stack */}
            <div
              style={{
                position: "absolute",
                top: 380,
            ...
            >
              <div style={{ fontSize: 18, color: "#94A3B8", fontFamily: NEMI_THEME.typography.fontCode }}>
                float(0.1) = 0.10000000000000000555...
              </div>
              <div style={{ fontSize: 18, color: "#94A3B8", fontFamily: NEMI_THEME.typography.fontCode }}>
                + float(0.2) = 0.20000000000000001110...
              </div>
              <div style={{ height: 2, backgroundColor: "rgba(255, 255, 255, 0.2)" }} />

              <div style={{ fontSize: 24, fontWeight: 900, color: "#FFFFFF", fontFamily: NEMI_THEME.typography.fontCode }}>
                = 0.3000000000000000<span style={{ color: isHighlight ? "#FFD166" : "#FFFFFF", fontSize: isHighlight ? 32 : 24, fontWeight: 900, textShadow: isHighlight ? "0 0 16px #FFD166" : "none" }}>4</span>
              </div>
            </div>
          </>
        );
      ...
      {/* ═══════════════════════════════════════════════════════════
          BEAT 5 & 6 — THE "AHA!" & OUTRO PAYOFF (16.5–22.0s)
         ═══════════════════════════════════════════════════════════ */}
