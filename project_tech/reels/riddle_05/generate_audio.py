#!/usr/bin/env python3
"""
Nemi Explains Reel #5 — "The Deadly Riddle: The Dining Philosophers & The System Deadlock"
Optimized strictly for High-Retention <25.5s Sweet Spot!
"""
...
PUBLIC_REELS = BASE_DIR / "public" / "reels" / "riddle_05"
...
# ═══════════════════════════════════════════════════════════════
# TIGHT, HIGH-RETENTION RIDDLE SCRIPT (STRICTLY <25.5s @ 30fps)
# ═══════════════════════════════════════════════════════════════
SPEAKER_EVENTS = [
    # 1. Beat 1: Frame-0 Instant Mystery Hook (No "computer science" jargon)
    {
        "id": "rd01_hook",
        "speaker": "narrator",
        "text": "Can you solve this deadly riddle?",
        "emotion": "dramatic",
        "exaggeration": 0.60,
        "gap_after_ms": 100,
        "semantic_phrases": [
            {"phrase": "deadly riddle", "cue": "hook_impact", "rel_pct": 0.50}
        ]
    },
    # 2. Beat 2: Clue 1 - The 5 Masters and 2 Chopsticks Rule
    {
        "id": "rd02_setup_masters",
        "speaker": "narrator",
        "text": "Five masters sit at a table. To eat, each master must hold two chopsticks.",
        "emotion": "normal",
        "exaggeration": 0.55,
        "gap_after_ms": 100,
        "semantic_phrases": [
            {"phrase": "Five masters", "cue": "table_spawn", "rel_pct": 0.20},
            {"phrase": "two chopsticks", "cue": "chopsticks_rule", "rel_pct": 0.75}
        ]
    },
    # 3. Beat 3: Clue 2 - Simultaneous Left Grab
    {
        "id": "rd03_left_grab",
        "speaker": "narrator",
        "text": "Every master simultaneously grabs the chopstick to their left.",
        "emotion": "normal",
        "exaggeration": 0.55,
        "gap_after_ms": 100,
        "semantic_phrases": [
            {"phrase": "simultaneously grabs", "cue": "left_grab_sweep", "rel_pct": 0.40},
            {"phrase": "to their left", "cue": "hold_one", "rel_pct": 0.80}
        ]
    },
    # 4. Beat 4: Clue 3 - The Fatal Starvation Trap (Tightened)
    {
        "id": "rd04_starvation_trap",
        "speaker": "narrator",
        "text": "Each master waits forever for the right chopstick, and all five starve without dropping theirs.",
        "emotion": "dramatic",
        "exaggeration": 0.58,
        "gap_after_ms": 100,
        "semantic_phrases": [
            {"phrase": "waits forever", "cue": "circular_arrows", "rel_pct": 0.30},
            {"phrase": "all five starve", "cue": "starvation_alert", "rel_pct": 0.70}
        ]
    },
    # 5. Beat 5: The Question & Countdown Trigger
    {
        "id": "rd05_countdown_prompt",
        "speaker": "narrator",
        "text": "What invisible curse just killed them all?",
        "emotion": "dramatic",
        "exaggeration": 0.60,
        "gap_after_ms": 80,
        "semantic_phrases": [
            {"phrase": "invisible curse", "cue": "timer_start", "rel_pct": 0.40}
        ]
    },
    # 6. Beat 6: Nemi Mascot Wrong Guess
    {
        "id": "rd06_nemi_guess",
        "speaker": "nemi",
        "text": "A curse from the chef?! 🤔",
        "emotion": "shocked",
        "exaggeration": 0.70,
        "gap_after_ms": 80,
        "semantic_phrases": [
            {"phrase": "from the chef", "cue": "buzzer_shock", "rel_pct": 0.60}
        ]
    },
    # 7. Beat 7: The Revelation & Real CS Mechanism (Deadlock / Thread Lock - Streamlined)
    {
        "id": "rd07_reveal_deadlock",
        "speaker": "narrator",
        "text": "It's a DEADLOCK. Thread A holds Lock one waiting for Lock two, while Thread B holds Lock two waiting for Lock one, freezing the system.",
        "emotion": "dramatic",
        "exaggeration": 0.60,
        "gap_after_ms": 90,
        "semantic_phrases": [
            {"phrase": "DEADLOCK", "cue": "deadlock_title", "rel_pct": 0.15},
            {"phrase": "Lock one waiting for Lock two", "cue": "thread_cycle", "rel_pct": 0.50},
            {"phrase": "freezing the system", "cue": "os_freeze", "rel_pct": 0.85}
        ]
    },
    # 8. Beat 8: Nemi Mascot Smug Payoff & Summary
    {
        "id": "rd08_nemi_payoff",
        "speaker": "nemi",
        "text": "Circular wait equals frozen servers! ❄️💀",
        "emotion": "smug",
        "exaggeration": 0.75,
        "gap_after_ms": 160,
        "semantic_phrases": [
            {"phrase": "frozen servers", "cue": "nemi_smug_stamp", "rel_pct": 0.50}
        ]
    }
]
...
def main():
    print("═" * 70)
    print("🎙️ NEMI EXPLAINS REEL #5 (TIGHT RETENTION SWEET SPOT <25.5s)")
    print("═" * 70)

    device = "mps" if torch.backends.mps.is_available() else "cpu"
    print(f"   Engine: Chatterbox Neural Expressive TTS + Edge-TTS AnaNeural")
    print(f"   Target Duration: ~24.5s")
    print(f"   BGM: Synthwave Goose - Blade Runner 2049.mp3")
    ...
    print("\n" + "─" * 70)
    print(f"⏱ Total Master Duration: {total_duration_s:.2f}s ({total_frames} frames @ 30fps) — PERFECT RETENTION WINDOW!")
    print("─" * 70 + "\n")
    ...
    raw_voice_wav = BLOCKS_DIR / "riddle_voice_raw.wav"
    ...
    final_voice_mp3 = Path(__file__).resolve().parent / "audio" / "riddle_voice_track.mp3"
    norm_cmd = [
    ...
    # 3. Dynamic Sidechain Ducking with Synthwave Goose - Blade Runner 2049.mp3
    ...
    master_audio_mp3 = PUBLIC_REELS / "riddle_master_audio.mp3"

    if bgm_path.exists():
        print(f"🎵 Mixing BGM (Audible Sidechain Groove): {bgm_path.name}")
        sidechain_filter = (
    ...
    shutil.copy(master_audio_mp3, PUBLIC_SOUNDS / "riddle_master_audio.mp3")
    ...
    cues_json_path = BASE_DIR / "src" / "data" / "riddle_05_cues.json"
    cues_data = {
        "reel_id": "riddle_05",
        "title": "The Deadly Riddle: The Dining Philosophers & The System Deadlock",
