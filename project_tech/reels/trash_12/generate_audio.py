#!/usr/bin/env python3
"""
Nemi Explains Reel #12 — "Your Trash Was Always a Lie."
How file deletion actually works (unlink + free-space marking + recovery windows).
...
PUBLIC_REELS = BASE_DIR / "public" / "reels" / "trash_12"
...
# ═══════════════════════════════════════════════════════════════
SPEAKER_EVENTS = [
    # 1. Beat 1: FRAME-0 HOOK — POV drop inside the viewer's own action
    {
        "id": "tr01_hook",
        "speaker": "narrator",
        "text": "POV: you just tapped Empty Trash.",
        "emotion": "normal",
        "exaggeration": 0.58,
        "gap_after_ms": 110,
        "semantic_phrases": [
            {"phrase": "tapped", "cue": "tap_hit", "rel_pct": 0.30},
            {"phrase": "Empty Trash", "cue": "bin_burst", "rel_pct": 0.70}
        ]
    },
    # 2. Beat 2: THE TWIST — loop opens
    {
        "id": "nc02_twist",
        "speaker": "narrator",
        "text": "Nothing got deleted. Your files are still sitting right there.",
        "emotion": "dramatic",
        "exaggeration": 0.60,
        "gap_after_ms": 100,
        "semantic_phrases": [
            {"phrase": "Nothing got deleted", "cue": "ghost_reform", "rel_pct": 0.40},
            {"phrase": "right there", "cue": "still_tags", "rel_pct": 0.85}
        ]
    },
    # 3. Beat 3: Nemi voices the viewer's question
    {
        "id": "nc03_nemi_guess",
        "speaker": "nemi",
        "text": "Wait... then where did my files go?!",
        "emotion": "shocked",
        "exaggeration": 0.70,
        "gap_after_ms": 90,
        "semantic_phrases": [
            {"phrase": "where did my files go", "cue": "nemi_shock", "rel_pct": 0.45}
        ]
    },
    # 4. Beat 4: THE SECRET
    {
        "id": "nc04_secret",
        "speaker": "narrator",
        "text": "They're still on the disk — marked as free space.",
        "emotion": "normal",
        "exaggeration": 0.55,
        "gap_after_ms": 100,
        "semantic_phrases": [
            {"phrase": "still on the disk", "cue": "hex_dive", "rel_pct": 0.30},
            {"phrase": "free space", "cue": "free_marker", "rel_pct": 0.75}
        ]
    },
    # 5. Beat 5: MECHANISM
    {
        "id": "nc05_mechanism",
        "speaker": "narrator",
        "text": "Until something new overwrites that space, recovery tools read every single byte.",
        "emotion": "normal",
        "exaggeration": 0.55,
        "gap_after_ms": 100,
        "semantic_phrases": [
            {"phrase": "overwrites", "cue": "scan_sweep", "rel_pct": 0.35},
            {"phrase": "every single byte", "cue": "byte_zoom", "rel_pct": 0.80}
        ]
    },
    # 6. Beat 6: PAYOFF (~55-58%)
    {
        "id": "nc06_payoff",
        "speaker": "narrator",
        "text": "So 'delete' really means invisible.",
        "emotion": "dramatic",
        "exaggeration": 0.62,
        "gap_after_ms": 110,
        "semantic_phrases": [
            {"phrase": "'delete'", "cue": "delete_stamp", "rel_pct": 0.25},
            {"phrase": "invisible", "cue": "wipe_invisible", "rel_pct": 0.65}
        ]
    },
    # 7. Beat 7: Nemi Smug Stamp
    {
        "id": "nc07_nemi_payoff",
        "speaker": "nemi",
        "text": "So my trash bin was always a lie?",
        "emotion": "smug",
        "exaggeration": 0.75,
        "gap_after_ms": 140,
        "semantic_phrases": [
            {"phrase": "always a lie", "cue": "smug_stamp", "rel_pct": 0.45}
        ]
    },
    # 8. Beat 8: LOOP SEAM
    {
        "id": "nc08_loop",
        "speaker": "narrator",
        "text": "Empty it again tomorrow — honestly, the files don't even care.",
        "emotion": "normal",
        "exaggeration": 0.55,
        "gap_after_ms": 120,
        "semantic_phrases": [
            {"phrase": "tomorrow", "cue": "loop_tap", "rel_pct": 0.30},
            {"phrase": "don't even care", "cue": "loop_fight", "rel_pct": 0.80}
        ]
    }
]
...
def main():
    print("═" * 70)
    print("🎙️ NEMI EXPLAINS REEL #12 — TRASH DELETION (19-22s WINDOW)")
    print("═" * 70)

    device = "mps" if torch.backends.mps.is_available() else "cpu"
    print(f"   Engine: Chatterbox Neural Expressive TTS + Edge-TTS AnaNeural")
    print(f"   BGM: NONE — SFX-only sound design experiment (music removed per v3 hypothesis)")
    ...
    raw_voice_wav = BLOCKS_DIR / "trash_voice_raw.wav"
    ...
    final_voice_mp3 = Path(__file__).resolve().parent / "audio" / "trash_voice_track.mp3"
    norm_cmd = [
    ...
    master_audio_mp3 = PUBLIC_REELS / "trash_master_audio.mp3"
    boost_cmd = [
        "ffmpeg", "-y",
    ...
    shutil.copy(master_audio_mp3, PUBLIC_SOUNDS / "trash_master_audio.mp3")
    ...
    cues_json_path = BASE_DIR / "src" / "data" / "trash_12_cues.json"
    cues_data = {
        "reel_id": "trash_12",
        "title": "Your Trash Was Always a Lie.",
