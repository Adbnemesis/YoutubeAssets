#!/usr/bin/env python3
"""
Nemi Explains Reel #10 — "Your Phone Is Silent. They Still Find You."
How GPS trilateration actually works (listen-only positioning + distance spheres + clock-bias fix).
...
PUBLIC_REELS = BASE_DIR / "public" / "reels" / "gps_10"
...
# ═══════════════════════════════════════════════════════════════
# GPS SCRIPT — payoff ~60%, loop seam, 19-22s window
# ═══════════════════════════════════════════════════════════════
SPEAKER_EVENTS = [
    # 1. Beat 1: FRAME-0 HOOK — silent phone, satellites locking on
    {
        "id": "gp01_hook",
        "speaker": "narrator",
        "text": "Your phone says nothing... and satellites still find you.",
        "emotion": "dramatic",
        "exaggeration": 0.62,
        "gap_after_ms": 110,
        "semantic_phrases": [
            {"phrase": "says nothing", "cue": "silent_mode", "rel_pct": 0.35},
            {"phrase": "find you", "cue": "spheres_lock", "rel_pct": 0.85}
        ]
    },
    # 2. Beat 2: The Secret — listen-only positioning
    {
        "id": "gp02_secret",
        "speaker": "narrator",
        "text": "It never sends a signal. It only listens for atomic timestamps.",
        "emotion": "normal",
        "exaggeration": 0.55,
        "gap_after_ms": 100,
        "semantic_phrases": [
            {"phrase": "never sends", "cue": "tx_off", "rel_pct": 0.25},
            {"phrase": "atomic timestamps", "cue": "timestamp_pop", "rel_pct": 0.80}
        ]
    },
    # 3. Beat 3: Nemi's Wrong Guess
    {
        "id": "gp03_nemi_guess",
        "speaker": "nemi",
        "text": "So it tracks my phone?! 🤯",
        "emotion": "shocked",
        "exaggeration": 0.70,
        "gap_after_ms": 90,
        "semantic_phrases": [
            {"phrase": "tracks my phone", "cue": "buzzer_shock", "rel_pct": 0.50}
        ]
    },
    # 4. Beat 4: The Reversal — distance spheres
    {
        "id": "gp04_reversal",
        "speaker": "narrator",
        "text": "Wrong! Every satellite paints one giant distance sphere around itself.",
        "emotion": "dramatic",
        "exaggeration": 0.60,
        "gap_after_ms": 100,
        "semantic_phrases": [
            {"phrase": "Wrong", "cue": "wrong_stamp", "rel_pct": 0.10},
            {"phrase": "distance sphere", "cue": "sphere_one", "rel_pct": 0.65}
        ]
    },
    # 5. Beat 5: The Mechanism — three spheres intersect
    {
        "id": "gp05_mechanism",
        "speaker": "narrator",
        "text": "Where three spheres cross... that's exactly where you're standing.",
        "emotion": "normal",
        "exaggeration": 0.55,
        "gap_after_ms": 100,
        "semantic_phrases": [
            {"phrase": "three spheres", "cue": "sphere_two", "rel_pct": 0.20},
            {"phrase": "where you're standing", "cue": "sphere_three", "rel_pct": 0.75}
        ]
    },
    # 6. Beat 6: THE PAYOFF — clock fix + pin drop (~60% of runtime)
    {
        "id": "gp06_payoff",
        "speaker": "narrator",
        "text": "A fourth satellite erases your chip's tiny clock error... pin dropped.",
        "emotion": "dramatic",
        "exaggeration": 0.62,
        "gap_after_ms": 110,
        "semantic_phrases": [
            {"phrase": "fourth satellite", "cue": "clock_fix", "rel_pct": 0.30},
            {"phrase": "pin dropped", "cue": "pin_drop", "rel_pct": 0.85}
        ]
    },
    # 7. Beat 7: Nemi Smug Stamp
    {
        "id": "gp07_nemi_payoff",
        "speaker": "nemi",
        "text": "Silence is the trick! 😎",
        "emotion": "smug",
        "exaggeration": 0.75,
        "gap_after_ms": 140,
        "semantic_phrases": [
            {"phrase": "Silence is the trick", "cue": "smug_stamp", "rel_pct": 0.40}
        ]
    },
    # 8. Beat 8: LOOP SEAM — flows back into the opening line
    {
        "id": "gp08_loop",
        "speaker": "narrator",
        "text": "So when your map nails your location instantly... remember, your phone never said a word.",
        "emotion": "normal",
        "exaggeration": 0.55,
        "gap_after_ms": 120,
        "semantic_phrases": [
            {"phrase": "nails your location", "cue": "loop_map", "rel_pct": 0.40},
            {"phrase": "never said a word", "cue": "loop_silent", "rel_pct": 0.82}
        ]
    }
]
...
def main():
    print("═" * 70)
    print("🎙️ NEMI EXPLAINS REEL #10 — GPS TRILATERATION (19-22s WINDOW)")
    print("═" * 70)
    ...
    raw_voice_wav = BLOCKS_DIR / "gps_voice_raw.wav"
    ...
    final_voice_mp3 = Path(__file__).resolve().parent / "audio" / "gps_voice_track.mp3"
    norm_cmd = [
    ...
    master_audio_mp3 = PUBLIC_REELS / "gps_master_audio.mp3"

    if bgm_path.exists():
        print(f"🎵 Mixing BGM with Dynamic Story Arc: {bgm_path.name}")
        # hook 0-22%: 0.34 | secret 22-38%: 0.26 | spheres 38-60%: 0.32 | climax 60-80%: 0.42 | resolve: 0.28
        t_hook_end = min(4.2, total_duration_s * 0.22)
        t_secret_end = min(7.8, total_duration_s * 0.38)
        t_build_end = min(12.5, total_duration_s * 0.60)
        t_climax_end = min(16.5, total_duration_s * 0.80)
        vol_expr = (
    ...
    shutil.copy(master_audio_mp3, PUBLIC_SOUNDS / "gps_master_audio.mp3")
    ...
    cues_json_path = BASE_DIR / "src" / "data" / "gps_10_cues.json"
    cues_data = {
        "reel_id": "gps_10",
        "title": "Your Phone Is Silent. They Still Find You.",
