#!/usr/bin/env python3
"""
Nemi Explains Reel #6 — "You Destroyed This QR Code. It Still Scanned."
Reed-Solomon Error Correction — Everyday Tech Mystery (data-ranked #1 topic).

Doctrine applied (26_RETENTION_POSTMORTEM_AND_PLAYBOOK.md):
- Frame-0 money shot (anomaly first, contradiction overlay)
- Payoff lands by ~55-60% of runtime
- Loop seam: final line returns to the opening image/line
- Duration target 19-22s, hard cap 24s
- BGM energy floor with dynamic story-arc envelope (V14 pattern)
"""
...
PUBLIC_REELS = BASE_DIR / "public" / "reels" / "qr_06"
...
# ═══════════════════════════════════════════════════════════════
# QR REEL SCRIPT — 52 spoken words, payoff by ~57%, loop seam
# ═══════════════════════════════════════════════════════════════
SPEAKER_EVENTS = [
    # 1. Beat 1: FRAME-0 HOOK — anomaly first, zero scene-setting
    {
        "id": "qr01_hook",
        "speaker": "narrator",
        "text": "You just destroyed this QR code... and it still scanned.",
        "emotion": "dramatic",
        "exaggeration": 0.62,
        "gap_after_ms": 110,
        "semantic_phrases": [
            {"phrase": "destroyed", "cue": "scribble_burst", "rel_pct": 0.35},
            {"phrase": "still scanned", "cue": "scan_check", "rel_pct": 0.82}
        ]
    },
    # 2. Beat 2: The Hidden Secret
    {
        "id": "qr02_secret",
        "speaker": "narrator",
        "text": "Every QR code secretly carries backup math of its own data.",
        "emotion": "normal",
        "exaggeration": 0.55,
        "gap_after_ms": 100,
        "semantic_phrases": [
            {"phrase": "secretly", "cue": "grid_reveal", "rel_pct": 0.40},
            {"phrase": "backup math", "cue": "gold_modules", "rel_pct": 0.78}
        ]
    },
    # 3. Beat 3: Nemi's Wrong Guess
    {
        ...
        "gap_after_ms": 90,
        "semantic_phrases": [
            {"phrase": "Spare copies", "cue": "buzzer_shock", "rel_pct": 0.45}
        ]
    },
    # 4. Beat 4: The Reversal — Reed-Solomon Mechanism
    {
        "id": "qr04_reversal",
        "speaker": "narrator",
        "text": "No spare room! Reed-Solomon equations rebuild missing blocks from whatever survives.",
        "emotion": "dramatic",
        "exaggeration": 0.60,
        "gap_after_ms": 100,
        "semantic_phrases": [
            {"phrase": "Reed-Solomon", "cue": "rs_title", "rel_pct": 0.22},
            {"phrase": "rebuild missing blocks", "cue": "laser_rebuild", "rel_pct": 0.58}
        ]
    },
    # 5. Beat 5: THE PAYOFF (lands ~57% of runtime)
    {
        "id": "qr05_payoff",
        "speaker": "narrator",
        "text": "Erase thirty percent — it reconstructs perfectly.",
        "emotion": "dramatic",
        "exaggeration": 0.62,
        "gap_after_ms": 110,
        "semantic_phrases": [
            {"phrase": "thirty percent", "cue": "meter_surge", "rel_pct": 0.38},
            {"phrase": "perfectly", "cue": "rebuilt_chime", "rel_pct": 0.85}
        ]
    },
    # 6. Beat 6: Nemi Smug Stamp
    {
        "id": "qr06_nemi_payoff",
        "speaker": "nemi",
        "text": "Math wins again! 😎",
        "emotion": "smug",
        "exaggeration": 0.75,
        "gap_after_ms": 140,
        "semantic_phrases": [
            {"phrase": "Math wins", "cue": "smug_stamp", "rel_pct": 0.40}
        ]
    },
    # 7. Beat 7: LOOP SEAM — final line flows back into the opening image
    {
        "id": "qr07_loop",
        "speaker": "narrator",
        "text": "So scribble away. Under that limit, it always scans.",
        "emotion": "normal",
        "exaggeration": 0.55,
        "gap_after_ms": 120,
        "semantic_phrases": [
            {"phrase": "scribble away", "cue": "scribble_again", "rel_pct": 0.28},
            {"phrase": "always scans", "cue": "loop_check", "rel_pct": 0.80}
        ]
    }
]
...
def main():
    print("═" * 70)
    print("🎙️ NEMI EXPLAINS REEL #6 — QR CODE REED-SOLOMON (19-22s WINDOW)")
    print("═" * 70)
    ...
    raw_voice_wav = BLOCKS_DIR / "qr_voice_raw.wav"
    ...
    final_voice_mp3 = Path(__file__).resolve().parent / "audio" / "qr_voice_track.mp3"
    norm_cmd = [
    ...
    # 3. Dynamic BGM Story Arc + Sidechain Ducking (Luminary — energetic violin-synth)
    ...
    master_audio_mp3 = PUBLIC_REELS / "qr_master_audio.mp3"

    if bgm_path.exists():
        print(f"🎵 Mixing BGM with Dynamic Story Arc: {bgm_path.name}")
        # V14 envelope doctrine, retimed to this script's beats:
        # hook 0-4s: 0.34 | grid secret 4-8s: 0.26 | reversal build 8-13s: 0.32 | climax swell 13-17s: 0.42 | payoff resolve 17s+: 0.28
    ...
    shutil.copy(master_audio_mp3, PUBLIC_SOUNDS / "qr_master_audio.mp3")
    ...
    cues_json_path = BASE_DIR / "src" / "data" / "qr_06_cues.json"
    cues_data = {
        "reel_id": "qr_06",
