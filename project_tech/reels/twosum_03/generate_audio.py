#!/usr/bin/env python3
"""
Nemi Explains Reel #3 — Master Speaker Orchestration Audio Pipeline
Topic: "Two Sum: The O(N^2) Trap vs The 1-Pass Hash Map" (~19.5s @ 30fps)
...
PUBLIC_REELS = BASE_DIR / "public" / "reels" / "twosum_03"
...
# ═══════════════════════════════════════════════════════════════
# REEL #3 MASTER SPEAKER TIMELINE (~19.5s Target)
# ═══════════════════════════════════════════════════════════════
SPEAKER_EVENTS = [
    # 1. Beat 1: Hook & Problem Setup
    {
        "id": "ts01_hook",
        "speaker": "narrator",
        "text": "This is the number one asked question in FAANG interviews: Two Sum.",
        "emotion": "dramatic",
        "exaggeration": 0.60,
        "gap_after_ms": 200,
        "semantic_phrases": [
            {"phrase": "number one", "cue": "array_enter", "rel_pct": 0.35},
            {"phrase": "Two Sum", "cue": "target_glow", "rel_pct": 0.75},
        ]
    },
    # 2. Beat 2: Brute Force Trap
    {
        "id": "ts02_brute",
        "speaker": "narrator",
        "text": "The naive mistake? Checking every pair with nested loops. That's O of N squared!",
        "emotion": "dramatic",
        "exaggeration": 0.65,
        "gap_after_ms": 200,
        "semantic_phrases": [
            {"phrase": "nested loops", "cue": "brute_scan_start", "rel_pct": 0.45},
            {"phrase": "N squared", "cue": "n_squared_alarm", "rel_pct": 0.80},
        ]
    },
    # 3. Beat 2b: Nemi Question
    {
        "id": "ts03_nemi_question",
        "speaker": "nemi",
        "text": "Can we do it in one pass?",
        "emotion": "normal",
        "exaggeration": 0.65,
        "gap_after_ms": 220,
        "semantic_phrases": [
            {"phrase": "one pass", "cue": "nemi_puzzled_pass", "rel_pct": 0.50}
        ]
    },
    # 4. Beat 3: Hash Map Complement Formula
    {
        "id": "ts04_formula",
        "speaker": "narrator",
        "text": "Yes! Use a Hash Map. For each number, calculate Target minus Current.",
        "emotion": "normal",
        "exaggeration": 0.55,
        "gap_after_ms": 200,
        "semantic_phrases": [
            {"phrase": "Hash Map", "cue": "hash_map_spawn", "rel_pct": 0.30},
            {"phrase": "Target minus Current", "cue": "formula_illuminate", "rel_pct": 0.75},
        ]
    },
    # 5. Beat 4: Live Resolution & Collision
    {
        "id": "ts05_resolution",
        "speaker": "narrator",
        "text": "At two, store index zero. At seven, nine minus seven is two... it's already in our map!",
        "emotion": "normal",
        "exaggeration": 0.60,
        "gap_after_ms": 220,
        "semantic_phrases": [
            {"phrase": "index zero", "cue": "map_insert_two", "rel_pct": 0.25},
            {"phrase": "nine minus seven", "cue": "calc_seven_two", "rel_pct": 0.60},
            {"phrase": "already in our map", "cue": "instant_collision_match", "rel_pct": 0.85},
        ]
    },
    # 6. Beat 5: Linear Time Payoff
    {
        "id": "ts06_payoff",
        "speaker": "narrator",
        "text": "Boom! Solved in O of N linear time.",
        "emotion": "happy",
        "exaggeration": 0.65,
        "gap_after_ms": 180,
        "semantic_phrases": [
            {"phrase": "linear time", "cue": "linear_payoff_glow", "rel_pct": 0.60}
        ]
    },
    # 7. Beat 5b: Nemi Celebration
    {
        "id": "ts07_nemi_smug",
        "speaker": "nemi",
        "text": "One pass, instant hire!",
        "emotion": "happy",
        "exaggeration": 0.80,
        "gap_after_ms": 300,
        "semantic_phrases": [
            {"phrase": "instant hire", "cue": "nemi_smug_hire", "rel_pct": 0.50}
        ]
    }
]
...
def main():
    print("═" * 70)
    print("🎙️ NEMI EXPLAINS REEL #3 — TWO SUM (LEETCODE #1) AUDIO PIPELINE")
    print("═" * 70)
    ...
    print(f"   Gaps: 180–220ms (Rapid tech flow)")
    ...
    for i, event in enumerate(SPEAKER_EVENTS, 1):
        ...
        if speaker == "nemi":
            # Generate snappy mascot voice via Edge-TTS AnaNeural with pitch=+12Hz, rate=+18%
    ...
    raw_voice_wav = BLOCKS_DIR / "twosum_voice_raw.wav"
    ...
    final_voice_mp3 = Path(__file__).resolve().parent / "audio" / "twosum_voice_track.mp3"
    norm_cmd = [
    ...
    master_audio_mp3 = PUBLIC_REELS / "twosum_master_audio.mp3"
    ...
    # Copy to public/sounds as well
    import shutil
    shutil.copy(master_audio_mp3, PUBLIC_SOUNDS / "twosum_master_audio.mp3")

    # 4. Export JSON cues for Remotion
    cues_json_path = BASE_DIR / "src" / "data" / "twosum_cues.json"
    cues_data = {
        "reel_id": "twosum_03",
        "title": "Two Sum: The O(N^2) Trap vs The 1-Pass Hash Map",
