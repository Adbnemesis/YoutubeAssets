...
def clean_text(text):
    text = text.lower()
    # Normalize common contractions and terms
...
def main():
    ...
    transcript_path = project_dir / "project_13_transcript"
    transcript_json_path = project_dir / "voiceover_transcript.json"

    # 1. Parse sentences from the script
    sentences = parse_sentences(script_path)
    print(f"Parsed {len(sentences)} sentences from script.")

    # 2. Check if we already have the Whisper transcript cached to save time
    ...
    # 3. Extract word list and times from transcript
    transcript_words = []
    transcript_word_times = []
    
    # We first check if the transcript has segment-level word lists
    for seg in transcript_data.get("segments", []):
        words = seg.get("words", [])
        if not words:
            # Fallback if Whisper did not output word timestamps per segment
            # We split the segment text and estimate times
    ...
    # 4. Flatten script words and link them back to sentence indices
    script_words = []
    word_to_sentence = []
    ...
    # 5. Use SequenceMatcher to find alignment mapping between script and transcript
    ...
    # Interpolate missing mappings to ensure every word has a time
    last_known = 0
    for script_idx in range(len(script_words)):
        if script_idx in script_to_transcript_map:
            ...
            # Look ahead for the next matched word
            next_s = None
            next_t = None
    ...
    # 6. Extract start and end times for each sentence
    sentence_times = {}
    ...
    # Ensure strictly non-overlapping and chronological times
