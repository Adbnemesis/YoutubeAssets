...
export interface KenjiCharacterProps {
  ...
  isSpeaking?: boolean;
  frame?: number;
  pose?: "idle" | "slash" | "sushi_slice" | "victorious" | "run" | "attack" | "win" | "lose" | "cool" | "sit" | "sheathe";
...
export const KenjiCharacter: React.FC<KenjiCharacterProps> = ({
  ...
  const poseBob = pose === "victorious" || pose === "win" || pose === "cool"
    ? Math.abs(Math.sin(frame * 0.2)) * 18
    : pose === "run"
      ? Math.abs(Math.sin(frame * 0.35)) * 10
      : pose === "sit"
        ? 0
        : 0;
  const bodyLean =
    pose === "attack" || pose === "slash" || pose === "sheathe"
      ? 14
      : pose === "run"
        ? -18
        : pose === "sit"
          ...
          : pose === "cool"
            ...
            : pose === "victorious"
              ? 0
              : 6;
  const armRaise = pose === "victorious" || pose === "win" || pose === "cool" ? -70 : pose === "sushi_slice" ? -40 : pose === "sheathe" ? -20 : 0;
  const headTilt = pose === "run" ? 14 : pose === "attack" ? -8 : pose === "cool" ? 18 : 0;

  // Slash Arc Animation
  const slashArc = attackProgress > 0 ? -Math.sin(attackProgress * Math.PI) * 110 : (pose === "slash" ? Math.sin(frame * 0.4) * 30 : pose === "sheathe" ? -40 : 0);
  ...
  // Sit pose: both knees folded forward, legs tucked under
  const sitting = pose === "sit" ? 1 : 0;
  const lHip = leftHipAngle !== undefined ? leftHipAngle : sitting ? 85 : legSin * 25;
  const lKnee = leftKneeAngle !== undefined ? leftKneeAngle : sitting ? 100 : Math.max(0, legSin * 40);

  const rHip = rightHipAngle !== undefined ? rightHipAngle : sitting ? -85 : -legSin * 25;
  const rKnee = rightKneeAngle !== undefined ? rightKneeAngle : sitting ? 100 : Math.max(0, -legSin * 40);

  const lShoulder = leftShoulderAngle !== undefined ? leftShoulderAngle : pose === "cool" ? 45 : 0;
  const lElbow = leftElbowAngle !== undefined ? leftElbowAngle : pose === "cool" ? -35 : pose === "sheathe" ? 45 : 15;
    ...
    <div style={{ position: "relative", width: calculatedWidth, height, display: "inline-block", transform: "perspective(1000px) rotateX(8deg)", ...style }}>
      <svg width="100%" height="100%" viewBox="0 0 300 420" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        >
          ...
          <g id="kenji-arms" transform={`rotate(${lShoulder + armRaise / 3}, 80, 210)`}>
            <rect x="68" y="210" width="28" height="38" rx="10" fill="url(#kenjiSkinGrad)" stroke="#09090B" strokeWidth="4" />
            <g transform={`translate(68, 245) rotate(${lElbow + armRaise / 2}) translate(-68, -245)`}>
              <rect x="66" y="245" width="32" height="35" rx="4" fill="#F8FAFC" stroke="#94A3B8" strokeWidth="3" />
            </g>
          </g>

          {/* Head & Face */}
          <g id="kenji-head" transform={`rotate(${headAngle + headTilt}, 150, 145)`}>
