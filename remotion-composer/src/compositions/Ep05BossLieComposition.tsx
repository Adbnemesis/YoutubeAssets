...
export interface AIShowdownProps {
  ...
  turns: SpeechTurn[];
  totalFrames: number;
  introAudioDurationFrames?: number;
}
...
export const Ep05BossLieComposition: React.FC<AIShowdownProps> = ({
  topic = "Will AI lie to my boss so that i can be with my girlfriend",
  introHook = [],
  turns = [],
  introAudioDurationFrames = 0,
}) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // Dynamic Intro Sequence Calculation
  const framesPerIntroWord = 12;
  const introDuration = introHook && introHook.length > 0 ? introHook.length * framesPerIntroWord + 15 : 0;
  
  // Adjust global frame relative to the intro sequence!
  const adjustedFrame = Math.max(0, frame - introDuration);

  // Find active turn based on adjusted frame
  const activeTurn = turns.find(
    (t) => adjustedFrame >= t.startFrame && adjustedFrame < t.startFrame + t.durationFrames
  ) || turns[0];

  const isMemeBreak = activeTurn ? activeTurn.isMemeBreak : false;
  const memeType = activeTurn ? (activeTurn as any).memeType : null;
  const memeText1 = activeTurn ? (activeTurn as any).memeText1 : null;
  const memeText2 = activeTurn ? (activeTurn as any).memeText2 : null;
  ...
  // Safe Camera Motion
  const turnProgressFrame = activeTurn ? adjustedFrame - activeTurn.startFrame : 0;
  ...
  // Micro Animations
  const talkBounce = Math.abs(Math.sin(adjustedFrame * 0.35)) * 10;
  const idleSway = Math.sin(adjustedFrame * 0.08) * 4;
  const starPulse = 1 + Math.sin(adjustedFrame * 0.12) * 0.06;
  ...
  return (
    ...
    >
      {introAudioDurationFrames > 0 && (
        <Audio src={staticFile("audio/ep05_will_ai_lie_to/intro_hook.wav")} volume={1.0} />
      )}
      {frame < introDuration && introHook && introHook.length > 0 && (
        <AbsoluteFill style={{ backgroundColor: "#000000", zIndex: 1000, justifyContent: "center", alignItems: "center" }}>
          {introHook.map((word, idx) => {
            const wordStartFrame = idx * framesPerIntroWord;
            const wordProgress = Math.max(0, frame - wordStartFrame);
            const wordScale = interpolate(spring({ fps, frame: wordProgress, config: { damping: 12, stiffness: 200 } }), [0, 1], [0.3, 1]);
            const wordOpacity = frame >= wordStartFrame ? 1 : 0;
            return (
              <div
                key={idx}
                style={{
                  position: "absolute",
                  color: idx % 2 === 0 ? "#FFD166" : "#EF4444",
                  fontSize: 160,
                  fontWeight: 900,
                  textTransform: "uppercase",
                  opacity: wordOpacity,
                  transform: `scale(${wordScale}) rotate(${idx % 2 === 0 ? -3 : 3}deg)`,
      ...
      {/* Audio sequences need to be offset by introDuration as well */}
      <Sequence from={introDuration}>
      {/* Dynamic React Corporate Background */}
      <AbsoluteFill
        style={{
      ...
      {turns.map((turn) => {
        ...
        return (
          <React.Fragment key={turnKey}>
            ...
            {!turn.isMemeBreak && (
              <Sequence
                from={introDuration + turn.startFrame}
                durationInFrames={15}
            ...
            {turn.voteTarget && !turn.isMemeBreak && (
              <Sequence
                from={introDuration + turn.startFrame + 10}
                durationInFrames={15}
      ...
      {isMemeBreak && memeType && (
        <AbsoluteFill style={{ zIndex: 100 }}>
          <MemeOverlay memeType={memeType} text1={memeText1} text2={memeText2} />
        </AbsoluteFill>
      )}
